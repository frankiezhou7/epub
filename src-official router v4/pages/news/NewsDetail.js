...
export default class NewsDetail extends React.Component {
    ...
    static need = [
      newsDetailActions.findNewsById,
    ...
    componentDidMount() {
        this.props.findRelatedNews({type: _.get(this.props.newsItem, ['type','_id']), _id:{$ne:this.props.newsItem._id}});
    }
        ...
        return (
            <div key="renderNewsDetail" style={(styles.middleLeft)}>
                ...
                <div style={{marginTop: 15}}>
                    <span
                        style={(styles.newsDetailTitle)}>{displayWithLimit(newsDetailDetail.title, 100)}</span>
                    <div style={{marginTop: 8}}>
                        <span style={(styles.newsDetailBaseInfo)}>From: {newsDetailDetail.from || "E-Ports"}</span>
                        <span
                            style={(styles.newsDetailBaseInfo)}>Category: {newsDetailDetail.type == undefined ? '--' : newsDetailDetail.type.value}</span>
                        <span style={(styles.newsDetailBaseInfo)}>
                            <span
                                style={(styles.right)}>{moment(newsDetailDetail.dateCreate).format('DD/MMM/YYYY')}</span>
                        </span>
                    </div>
                </div>
    ...
    renderHotNews(styles) {
        ...
        let relatedNewsElem = relatedNews == 0 ?
            <div style={(styles.noRecommended)}>Sorry, No Related News...</div> : _.map(relatedNews, item => {
                return (

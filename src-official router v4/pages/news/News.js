...
@connect(
    ...
    dispatch => ({
        ...bindActionCreators({
            ...
            findNewsTypes: newsActions.findNewsTypes,
            findNewsBanners: newsActions.findNewsBanners,
        }, dispatch)
    })
)
...
export default class News extends React.Component {
    ...
    static defaultProps = {
        ...
        recommendationNews: [],
        fileUrl: '/files/',
        defaultType: {_id: 0, value: 'Latest News'}
    };

    componentWillMount() {
        /* this.props.findNewsBanners();
         this.props.findNews(); //查找latest News
         this.props.findNewsTypes(); //查找除latest News之外的新闻 类型
         this.props.findRecommendableNews(); //查找右侧推荐新闻*/
    }
    ...
    createCategoryElem = (styles) => {
        ...
        categories.map((item, i) => {
            if (i < maxShow) {
                ...
                dropDownCategory.push(
                    <MenuItem value={item._id} primaryText={displayWithLimit(item.value, 11)}/>
                );
            }
        });
        ...
        let categoryElem = [];
        categoryElem.push(
            <div key='categoryContainer' style={(styles.categoryContainer)}>{showCategory}</div>
        );
        if (dropDownCategory.length > 0) {
            categoryElem.push(
                <div style={(styles.categoryContainerDownDrop)}>
                    <IconMenu
    ...
    renderNews = (styles) => {
        ...
        if (this.state.isLoading) {
            ...
            let noContent = news && news.length == 0;
            newsElem = noContent ?
                <div style={(styles.noContent)}>No News...</div> : _.map(news, item => {
                    return (
    ...
    renderHotNews = (styles) => {
        let recommendationNews = this.props.recommendationNews != null ? this.props.recommendationNews : [];
        let hotNewsContent = recommendationNews.length == 0 ?
            <div style={(styles.noRecommended)}>No Data...</div> : _.map(recommendationNews, item => {
                return (
    ...
    _handleMoreScroll = () => {
        ...
        if (this.state.isLoad && document.body.scrollHeight <= (document.body.clientHeight + document.body.scrollTop + 180) && hasNext) {

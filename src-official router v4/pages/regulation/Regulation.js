...
export default class Regulation extends React.Component{
  ...
  componentDidMount = () => {
    if(this.props.regulations && this.props.regulations.length === 0){
      this.setState({hasRegulation: false});
    }

    if(this.props.recommendationRegulations && this.props.recommendationRegulations.length === 0){
      this.setState({hasRecommendation: false});
    }
  }
  ...
  renderRegulations = (styles) => {
    ...
    let recommendation = this.state.hasRecommendation ? _.map(recommendationRegulations, (item, index) => {
      return (
    ...
    }) : (<div style={styles.recommendation.noContent}>Sorry, No Recommended Regulations...</div>);

    let regulationWrapper = (
      <div style={styles.info}>
        <div id='regulationWrapper' style={styles.regulation.wrapper}>
          <div>
            {this.state.hasRegulation ? _.map(regulations, (item, index) => {
  ...
  _handleTitleTouchTap = (regulation)=>{
    this.props.history.push(`/regulation/${regulation._id}`);
  }

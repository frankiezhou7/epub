...
export default class PortCanvas extends React.Component{
  ...
  constructor(props,context){
    ...
    }else if(params.anchorageId){
      tabValue = TAB_VALUE.ANCHORAGE;
      anchorageId = params.terminalId;
    }
    this.state = {
      value: tabValue,
  ...
  componentWillReceiveProps(nextProps) {
    if(nextProps.user !== this.props.user){
      if(!__SERVER__ && this.props.port && this.props.port.visibleStatus === 1 && !this.isLoggedIn(nextProps)) {
  ...
  handleChange = (value) =>{
    this.setState({value: value},()=>{
      ...
      switch (value) {
        case TAB_VALUE.PORT:
            if(portId) this.props.history.push(`/port/${portId}`);
          break;
        case TAB_VALUE.TERMINAL:
            if(terminalId) this.props.history.push(`/port/${portId}/terminal/${terminalId}`);
          break;
        case TAB_VALUE.ANCHORAGE:
            if(anchorageId) this.props.history.push(`/port/${portId}/anchorage/${anchorageId}`);
          break;
        default:
          this.props.history.push(`/port/${portId}`);
      }
    });
  }

  handleSummaryCardTouchTap = (event,item)=>{
    this.props.history.push(`/organization/${item._id}`);
  }

  handleTeminalChange = (event, index, value)=>{
    this.props.history.push(`/port/${this.props.port._id}/terminal/${value}`);
  }

  handleAnchorageChange = (event, index, value)=>{
    this.props.history.push(`/port/${this.props.port._id}/anchorage/${value}`);
  }

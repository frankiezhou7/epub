...
export default class MenuCrumbs2 extends React.Component {
    ...
    static propTypes = {
        ...
        activedColor: React.PropTypes.string,
    };

    static defaultProps = {
      separator: '>',
      activedColor: 'rgba(0, 0, 0, 0.87)',
    };
    ...
    renderNavigation(styles) {
        let navElem = [];
        let { navArr, separator, activedColor } = this.props;
        navArr.map((nav, i) => {
            if (nav.link != undefined) {
                ...
                navElem.push(<span key={`nav${i}`} style={{color: i == navArr.length - 1 ? activedColor : 'rgba(0, 0, 0, 0.87)'}}>{nav.value}</span>)
            }
            if (i != navArr.length - 1) {
                navElem.push(<span key={`nav${i}_`}>&nbsp;&nbsp;{`${separator}`}&nbsp;&nbsp;</span>)
            }
        });
        return navElem;
    }
